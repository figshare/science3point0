...
function bp_gf_forums_load_bbpress() {
    if(function_exists('bp_forums_load_bbpress'))
        return;//do not try to re do the things if bp forums is active
...
add_action( 'bbpress_init', 'bp_gf_forums_load_bbpress' );

/* WP to bbP wrapper functions */
if(!function_exists("bb_get_current_user")&&!bp_is_active("forums")):
    function bb_get_current_user() { global $current_user; return $current_user; }
endif;
if(!function_exists("bb_get_user")&&!bp_is_active("forums")):
    function bb_get_user( $user_id ) { return get_userdata( $user_id ); }
endif;
if(!function_exists("bb_cache_users")&&!bp_is_active("forums")):
    function bb_cache_users( $users ) {}
endif;
/**
 * bbPress needs this class for its usermeta manipulation.
 */
if(!class_exists("BP_Forums_BB_Auth")):
